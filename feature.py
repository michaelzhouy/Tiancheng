...
class Feature:
    ...
    def get_feature(op, trans, label, w2v, ifdrop):
        ...
        if ifdrop:
            ...
            trans_feature = ['UID', 'channel', 'amt_src1', 'code1', 'trans_type1', 'device_code1', 'device_code2',
                             'device_code3', 'ip1', 'amt_src2', 'merchant', 'geo_code', 'trans_type2', 'ip1_sub']
        else:
            op_feature = op.columns
            op_deliver = ['ip1', 'ip1_sub', 'wifi', 'mac1', 'mac2', 'geo_code', 'mode', 'device1', 'device2',
            ...
            if feature not in ['day']:
                ...
                for deliver in op_deliver:
                    if feature not in deliver:
                        if feature != 'UID':
                            temp = op[['UID', deliver]].merge(op.groupby([deliver])[feature].count().reset_index(),
                            ...
                            del temp
                            temp = op[['UID', deliver]].merge(op.groupby([deliver])[feature].nunique().reset_index(),
                                                              on=deliver, how='left')[['UID', feature]]
                            temp = temp.groupby('UID')[feature].sum().reset_index()
                            temp.columns = ['UID', feature + deliver]
                            label = label.merge(temp, on='UID', how='left')
